...
export default function Home() {
  ...
  const mouseTarget = useRef({ x: -200, y: -200 });
  const followerPos = useRef({ x: -200, y: -200 });
  const [followerShown, setFollowerShown] = useState(false);
  ...
  useEffect(() => {
    const onMouseMove = (e: MouseEvent) => {
      ...
      setFollowerShown(true);
    };
    const onMouseLeave = () => setFollowerShown(false);
    window.addEventListener("mousemove", onMouseMove);
    document.documentElement.addEventListener("mouseleave", onMouseLeave);

    let raf: number;
    const animate = () => {
      followerPos.current.x += (mouseTarget.current.x - followerPos.current.x) * 0.1;
      followerPos.current.y += (mouseTarget.current.y - followerPos.current.y) * 0.1;
  ...
  const storyLines = [
    { id: "s1", text: "Stel, je gaat naar een geweldig festival of evenement met je favoriete mensen..." },
    { id: "s2", text: "Hoe leuk als je match daar ook is." },
    { id: "s3", text: "Met Festiv match je van te voren met mensen die ook gaan. Of tijdens." },
    { id: "s4", text: "Spreek spontaan af linksvoor bij een stage, of bij dat ene stukje waar je lekker kunt zitten. En wie weet wordt een mooie dag nóg mooier!" },
  ];
    ...
    <main>
      ...
      <div
        ...
        style={{
          ...
          cursor: "pointer",
          userSelect: "none",
          opacity: followerShown ? 1 : 0,
          transition: "opacity 0.3s ease",
          boxShadow: "0 4px 20px rgba(42,23,88,0.3)",
          left: "-200px",
          top: "-200px",
        }}
      >
      ...
      {storyLines.map((line, i) => (
        <section key={line.id} style={{
          ...
          background: "#2A1758",
          padding: "80px 24px",
          borderTop: i > 0 ? "1px solid rgba(192,183,234,0.08)" : "none",
        }}>
          <div
            data-reveal={line.id}
            style={{
          ...
          >
            <p style={{
              fontFamily: i === 1 ? "'Playfair Display', serif" : "'Inter', sans-serif",
              fontSize: i === 1
                ? "clamp(36px, 6vw, 64px)"
                : i === 0
                  ? "clamp(20px, 3.5vw, 32px)"
                  : "clamp(22px, 4vw, 40px)",
              fontWeight: i === 1 ? 700 : i === 0 ? 300 : 500,
              color: i === 1 ? "#FFD166" : "#FFFFFF",
              lineHeight: 1.4,
      ...
      {/* Zo werkt het */}
      <section style={{ background: "#2A1758", padding: "100px 24px", borderTop: "1px solid rgba(192,183,234,0.08)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: "24px" }}>
            {[
              ...
              <div key={item.step} style={{
                ...
                <span style={{ position: "absolute", top: "24px", right: "24px", fontSize: "12px", opacity: 0.2, fontWeight: 700, color: "#FFFFFF" }}>
                  {item.step}
                </span>
